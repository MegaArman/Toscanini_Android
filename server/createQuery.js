...
module.exports = (searchString) =>
{
    ...
    if (queryPart.length === 1)
    {
      if ("composer" in queries)
      {
        if (queryPart[0].length != 0)
          errorString = 
          queryPart[0] + "- we only support querying for one composer at this time, please fix";
        else
  ...
  //  console.log("firstASCIICharMin", firstASCIICharMinPitch);
    ...
    {
      validQuery = false;
      errorString = (queryPart[0] + " -minimum should not be greater than maximum, please fix!"); 
      return;
    }
